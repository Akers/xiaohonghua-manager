<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="FamilyUserList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="租户号" align="center" prop="id" />
...
<script>
import { listFamilyUser, getFamilyUser, delFamilyUser, addFamilyUser, updateFamilyUser } from "@/api/family/FamilyUser";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFamilyUser(this.queryParams).then(response => {
        this.FamilyUserList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加家庭用户关联表";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getFamilyUser(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改家庭用户关联表";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateFamilyUser(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFamilyUser(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除家庭用户关联表编号为"' + ids + '"的数据项？').then(function() {
        return delFamilyUser(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('family/FamilyUser/export', {
        ...this.queryParams
